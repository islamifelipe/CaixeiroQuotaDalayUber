...
int main (int argc, char *argv[]){
	...
	cout<<"Semente utilizada : "<<ttt<<endl;
	srand (ttt);
	...
	for (int i=0; i<n; i++){
		...
		if (min<1) min=1; // no minino 1 por vértice
		int max = tetoPassageiros[rand()%3];
		int quant = min; // no minimo
		if (quant>max) quant = max; // no maximo
		...
		for (int j=0; j<passageiros[i].size(); j++){
			for (int vv = 0; vv<n; vv++){ // penalidade de deixar o passageiro pas_id2 em v
				if (vv == passageiros[i][j]){
					...
					int w = rand()%((int)(avg*0.4) - (int)(avg*0.2) + 1) + (int)(avg*0.2);
